...
public class GitIngestServiceImpl implements GitIngestService {
    ...
    public IngestResult ingest(String repoUrl,
        ...
        try {
            ...
            // 固定克隆路径：{baseDir}/git-ingest/{repoName}
            Path target = Paths.get(baseDir, "git-ingest", repoName);
            Files.createDirectories(target.getParent());

            // 若已存在且包含 .git，跳过克隆
            boolean alreadyCloned = Files.exists(target) && Files.exists(target.resolve(".git"));
            if (!alreadyCloned) {
                ...
                log.info("已存在克隆仓库，跳过克隆：{}", target);
            }

            // 扫描文件（排除 .git 目录）
            List<Path> files = listFilesExcludingGit(target);
    ...
    private static List<Path> listFilesExcludingGit(Path root) throws IOException {
        List<Path> all = new ArrayList<>();
        Files.walk(root)
                .filter(Files::isRegularFile)
                .filter(p -> !isGitInternalPath(p))
                .forEach(all::add);
        return all;
    }

    private static boolean isGitInternalPath(Path p) {
        // 跨平台判断路径中是否包含 .git 目录
        String s = p.toString();
        return s.contains(File.separator + ".git" + File.separator)
                || s.endsWith(File.separator + ".git")
                || s.contains("/.git/")
                || s.endsWith("/.git");
    }
