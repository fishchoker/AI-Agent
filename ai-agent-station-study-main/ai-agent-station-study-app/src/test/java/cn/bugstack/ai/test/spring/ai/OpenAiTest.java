...
import cn.bugstack.ai.config.AiAgentAutoConfiguration;
...
public class OpenAiTest {
	 ...
	 * 
	 * Flux<ChatResponse> stream = chatModel.stream(new Prompt( "1+1",
	 * OpenAiChatOptions.builder() .model("glm-4.5") .build()));
	 * 
	 * stream.subscribe( chatResponse -> { AssistantMessage output =
	 * chatResponse.getResult().getOutput(); log.info("测试结果(stream): {}",
	 * JSON.toJSONString(output)); }, Throwable::printStackTrace, () -> {
	 * countDownLatch.countDown(); log.info("测试结果(stream): done!"); } );
	 * 
